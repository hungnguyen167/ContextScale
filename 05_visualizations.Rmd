...
manifesto_cs_topic <- read_csv("results/datasets/ensemble_full_dataset.csv")
manifesto_org <- read_csv("data/py_outputs/manifesto.csv") 
...
plot_cor <- ggplot(data = cor_table_all, aes(x = topic, y = abs(cor), fill = factor(techniques))) +
    geom_bar(stat = "identity", position = position_dodge(width = 0.8), color = "black") +
