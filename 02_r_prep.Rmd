...
ches_deu <- ches %>%
    ...
    summarise(
        ...
        ches_redis = mean(redistribution, na.rm = TRUE),
        ches_sptax = mean(spendvtax, na.rm=TRUE)
    ) %>%
    ungroup() 

ches_deu <- ches_deu %>%
    mutate(
        ches_wf = rowMeans(select(ches_deu,c(ches_redis, ches_sptax)),
                           na.rm = TRUE)
    )
...
## Wordfish - Decentralization      
```{r wf_regions}

manifesto_regions = manifesto_deu[which(manifesto_deu$code_extract %in% c(301,302,305)),]
grouped_texts <- manifesto_regions %>% 
    mutate(
        party_election = paste(party, election, sep="_"),
        party = case_when(
            grepl("Linke|Sozialismus", name) ~ "PDS/Die Linke",
...
write.csv(results, "data/r_outputs/wf_regions.csv")
...
write.csv(results, "data/r_outputs/wf_econ.csv")
...
write.csv(results, "data/r_outputs/wf_socwel.csv")
...
write.csv(results, "data/r_outputs/wf_eu.csv")
...
write.csv(results, "data/r_outputs/wf_imm.csv")
