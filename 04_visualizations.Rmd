...
## Scaled datasets
manifesto_cs <- read_csv("data/py_outputs/cs_germany.csv")
...
```{r}
manifesto_aggr <- manifesto_cs %>%
    group_by(party, election) %>%
    summarise(
        position_scores = mean(lr_score, na.rm=TRUE)
    )
manifesto_topic_aggr <- manifesto_cs %>% 
    group_by(party, election, pred_topic_name) %>%
    summarise(
        position_scores = mean(lr_score, na.rm=TRUE)
    )
