...
cagree_full <- read_csv("data/py_outputs/cagree_scaled_full.csv")
manifesto_deu <- read_csv("data/temps/manifesto.csv") %>%
    filter(countryname=="Germany")

## Prediction results

manifesto_testset <- read_csv("results/classification results/manifesto_test_pred.csv")
manifesto_diflang <- read_csv("results/classification results/manifesto_lang_pred.csv")
cagree_nosup <- read_csv("results/classification results/cagree_nosup_pred.csv")
cagree_25sup <- read_csv("results/classification results/cagree_25sup_pred.csv")
motn_25sup <- read_csv("results/classification results/motn_25sup_pred.csv")
...
## F1 scores
coalition_pred_quality <- cagree_25 %>%
    mutate(
        true_lrn = case_when(
            lrn == "left" ~ 0,
            lrn == "neutral" ~ 1,
        ...
        true_topic = case_when(
            topic == 'Agriculture' ~ 0,
            topic == 'Civil Rights' ~ 1,
            topic == 'Decentralization' ~ 2,
            topic == 'Defense' ~ 3,
            topic == 'Economy' ~ 4,
            topic == 'Education' ~ 5,
            topic == 'Environment' ~ 6,
            topic == 'European integration' ~ 7,
            topic == 'Immigration' ~ 8,
            topic == 'International politics' ~ 9,
            topic == 'Other' ~ 10,
            topic == 'Social Welfare' ~ 11
        )
    )
cm_lrn <- confusionMatrix(factor(coalition_pred_quality$true_lrn), 
                      factor(coalition_pred_quality$pred_lr_25),
                      mode = "everything")
cm_topic <- confusionMatrix(factor(coalition_pred_quality$true_topic), 
                      factor(coalition_pred_quality$pred_topics_25),
                      mode = "everything")

mean_f1_lrn <- mean(cm_lrn[["byClass"]][,"F1"])
...
## Table 1
```{r table_1}
cm_mtest_topic <- round(confusionMatrix(factor(manifesto_testset$pred_topic), 
                      factor(manifesto_testset$topic),
                      mode = "everything")[["byClass"]],3)
cm_mtest_lrn <- round(confusionMatrix(factor(manifesto_testset$pred_lrn), 
                      factor(manifesto_testset$lrn),
                      mode = "everything")[["byClass"]],3)


cm_mlang_topic <- round(confusionMatrix(factor(manifesto_diflang$pred_topic), 
                      factor(manifesto_diflang$topic),
                      mode = "everything")[["byClass"]],3)
cm_mlang_lrn <- round(confusionMatrix(factor(manifesto_diflang$pred_lrn), 
                      factor(manifesto_diflang$lrn),
                      mode = "everything")[["byClass"]],3)

cm_cagree25_topic <- round(confusionMatrix(factor(cagree_25sup$pred_topic), 
                      factor(cagree_25sup$topic),
                      mode = "everything")[["byClass"]],3)
cm_cagree25_lrn <- round(confusionMatrix(factor(cagree_25sup$pred_lrn), 
                      factor(cagree_25sup$lrn),
                      mode = "everything")[["byClass"]],3)
cm_cagreenosup_topic <- round(confusionMatrix(factor(cagree_nosup$pred_topic), 
                      factor(cagree_nosup$topic),
                      mode = "everything")[["byClass"]],3)
cm_cagreenosup_lrn <- round(confusionMatrix(factor(cagree_nosup$pred_lrn), 
                      factor(cagree_nosup$lrn),
                      mode = "everything")[["byClass"]],3)

cm_motn25_lrn <- list(class0=round(confusionMatrix(factor(motn_25sup$pred_stances), 
                      factor(motn_25sup$trump_stance_auto),
                      mode = "everything", positive = "0")[["byClass"]],3),
                      class1=round(confusionMatrix(factor(motn_25sup$pred_stances), 
                      factor(motn_25sup$trump_stance_auto),
                      mode = "everything", positive = "1")[["byClass"]],3))



cls_res <- tibble(
    Metric = c("F1", "Precision", "Recall", "Accuracy"),
    `Model 1_Topic` = c(
        round(mean(cm_mtest_topic[,"F1"]),2),
        round(mean(cm_mtest_topic[,"Precision"]),2),
        round(mean(cm_mtest_topic[,"Recall"]),2),
        round(mean(cm_mtest_topic[,"Balanced Accuracy"]),2)
    ),
    `Model 1_Ideology` = c(
        round(mean(cm_mtest_lrn[,"F1"]),2),
        round(mean(cm_mtest_lrn[,"Precision"]),2),
        round(mean(cm_mtest_lrn[,"Recall"]),2),
        round(mean(cm_mtest_lrn[,"Balanced Accuracy"]),2)
    ),
    `Model 2_Topic` = c(
        round(mean(cm_mlang_topic[,"F1"]),2),
        round(mean(cm_mlang_topic[,"Precision"]),2),
        round(mean(cm_mlang_topic[,"Recall"]),2),
        round(mean(cm_mlang_topic[,"Balanced Accuracy"]),2)
    ),
    `Model 2_Ideology` = c(
        round(mean(cm_mlang_lrn[,"F1"]),2),
        round(mean(cm_mlang_lrn[,"Precision"]),2),
        round(mean(cm_mlang_lrn[,"Recall"]),2),
        round(mean(cm_mlang_lrn[,"Balanced Accuracy"]),2)
    ),
    `Model 3_Topic` = c(
        round(mean(cm_cagreenosup_topic[,"F1"]),2),
        round(mean(cm_cagreenosup_topic[,"Precision"]),2),
        round(mean(cm_cagreenosup_topic[,"Recall"]),2),
        round(mean(cm_cagreenosup_topic[,"Balanced Accuracy"]),2)
    ),
    `Model 3_Ideology` = c(
        round(mean(cm_cagreenosup_lrn[,"F1"]),2),
        round(mean(cm_cagreenosup_lrn[,"Precision"]),2),
        round(mean(cm_cagreenosup_lrn[,"Recall"]),2),
        round(mean(cm_cagreenosup_lrn[,"Balanced Accuracy"]),2)
    ),
    `Model 4_Topic` = c(
        round(mean(cm_cagree25_topic[,"F1"]),2),
        round(mean(cm_cagree25_topic[,"Precision"]),2),
        round(mean(cm_cagree25_topic[,"Recall"]),2),
        round(mean(cm_cagree25_topic[,"Balanced Accuracy"]),2)
    ),
    `Model 4_Ideology` = c(
        round(mean(cm_cagree25_lrn[,"F1"]),2),
        round(mean(cm_cagree25_lrn[,"Precision"]),2),
        round(mean(cm_cagree25_lrn[,"Recall"]),2),
        round(mean(cm_cagree25_lrn[,"Balanced Accuracy"]),2)
    ),
    `Model 5_Ideology` = c(
        round(mean(c(cm_motn25_lrn$class0["F1"],cm_motn25_lrn$class1["F1"])),2),
        round(mean(c(cm_motn25_lrn$class0["Precision"],cm_motn25_lrn$class1["Precision"])),2),
        round(mean(c(cm_motn25_lrn$class0["Recall"],cm_motn25_lrn$class1["Recall"])),2),
        round(mean(c(cm_motn25_lrn$class0["Balanced Accuracy"],cm_motn25_lrn$class1["Balanced Accuracy"])),2)
    )
...
gtsave(table_1, file = "results/classification results/table 1.docx")
