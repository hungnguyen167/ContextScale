...
plot_wf_eu <- ggplot(data = df_eu_deu,
    ...
    theme(axis.title = element_text(size = 15),legend.title = element_text(size = 15), legend.text = element_text(size = 14), title =  element_text(size = 18, face = "bold")) +
    xlab("Election") +
    ylab("Wordfish - European Integration") 
...
plot_d2v_eu <- ggplot(data = df_eu_deu,
                  aes(x=factor(election), y=d2v_d1, color=party,group=party)) +
    geom_point() +
    geom_line(alpha=0.8, linewidth = 1) + 
    scale_color_manual(name = "Party",  
...
plot_cmplog_eu <- ggplot(data = df_eu_deu,
                  aes(x=factor(election), y=lr_log, color=party,group=party)) +
    geom_point() +
    geom_line(alpha=0.8, linewidth = 1) + 
    scale_color_manual(name = "Party",  
    ...
    theme(axis.title = element_text(size = 15),legend.title = element_text(size = 15), legend.text = element_text(size = 14), title =  element_text(size = 18, face = "bold")) +
    xlab("Election") +
    ylab("MARPOR Log - European Integration") 

figure_1 <- ggarrange(
      plot_wf_ep, plot_d2v_ep, plot_cmplog_ep,
      plot_wf_eu, plot_d2v_eu, plot_cmplog_eu,
      common.legend = TRUE
)
...
agg_png(filename = here::here("results","tabs and figs","figure 2.png"), res = 360, width = 3600, height = 3000)
figure_2
invisible(dev.off())
...
cagree_unified <- cagree_all %>%
    dplyr::select(position_scores) %>%
    rename(position_scores_all = position_scores) %>%
    bind_cols(cagree_10) %>%
    rename(position_scores_10 = position_scores)

cagree_mean_topic <- cagree_unified %>%
    group_by(country, year, topic) %>%
    summarise(
        mean_10 = mean(position_scores_10, na.rm=TRUE),
        mean_full = mean(position_scores_all, na.rm=TRUE)
    ) 
...
country_agg <- cagree_unified %>%
    group_by(country) %>%
    summarise(
        mean_full = mean(position_scores_all),
        mean_10 = mean(position_scores_10),
    )
...
mean_cor <- cagree_unified %>%
    group_by(country) %>%
    summarise(
        cor_scores = cor(position_scores_all, position_scores_10)
    ) 
...
cm_sentiment <- confusionMatrix(factor(cagree_10$sentiment), 
                      factor(cagree_10$pred_sentiment),
                      mode = "everything")
cm_topic <- confusionMatrix(factor(cagree_10$topic), 
                      factor(cagree_10$pred_topic),
                      mode = "everything")
...
plot_corr <- ggplot(data = cagree_mean_topic,
                  aes(x=mean_full, y=mean_10, color=factor(country))) +
